...
## 2026-09-07 マニュアル更新 ＋ エミュレーション事前確認

### 完了

- マニュアル更新（v0.1.231 `e99d935` / v0.1.232 `33abc61`）
  - `docs/00`,`02`,`03`,`04`,`11` ＋ `templates/help.html` を進捗バー・Markdown 対応・
    ボタン名の実態合わせで更新。
  - `docs/04` に「スマホ実機でしか確認できない項目」「Claude が事前チェックできる範囲」を追加。
- `mp-quote`/`mp-code` インラインマーク追加（v0.1.232）でページ切替モードでも
  Markdown 引用の見た目が残るように。デモ `books/works/markdown-sample.md` を実文へ書き直し。

### エミュレーション事前確認（`tsukuyomi-reader-preview.pages.dev` v0.1.233）

- 390px / 834px で確認。EPUB 本文幅 約89〜92%、進捗バー動作、
  Markdown（見出し・太字・引用スタイル・区切り・ルビ）表示OK、コンソールエラーなし。
- 発見: ページ切替モードで先頭 `#` 見出しに本文ブロックが無いとほぼ空白の1ページになる。
  → デモに導入文を追加して回避（v0.1.233 `daec201`）。normalize-md 側での
  「空セクションを次章へ畳む」恒久対応は別途判断。
- タップ／ページ送りの向きはソース確認済み: 縦書き=`pageDirection:"rtl"` で
  左タップ／左スワイプ＝進む、右＝戻る、ホイールも一致。和書リーダー標準と同じ。

### 次にやること

- 有名ビューワー比較（`docs/53` 案）: モバイルのタップ領域配分（現状は外側25%のみページ送り、
  中央50%がメニュー）を i文庫式サードに寄せるか要判断。RTL 向き・章ジャンプ・
  ページめくり効果は既に一般作法どおり。
