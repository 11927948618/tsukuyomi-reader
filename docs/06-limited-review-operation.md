...
- R2の直リンク公開は避ける
- 限定レビュー用R2と一般公開用R2は分ける
- 一般公開は限定作品からの期限付き昇格コピーだけで行う
- 閲覧データは読者同士には公開せず、管理側で一元保管・集計し、将来の文芸分析や作品改善に使うことを案内文に明記する
- SNS共有・再配布・引用禁止を明記する

限定レビュー版URLは公開版とは別名にします。
...
- パスワードは発行から7日を更新目安にします。実際には再発行または無効化するまで使えます
- ログインセッションは14日で期限切れ
...
- 有効なメールアドレスまたは仮IDに対するPW失敗、ロック発生、PW発行、PW発行失敗、PW無効化、PW無効化失敗、同時利用検知は詳細イベントとして残す。旧仕様のPW期限切れ記録が残る場合があります
...
1. 限定レビュー用R2に保存する
2. 公開版Readerが読むR2には直接置かない
3. 一般公開する場合だけ、管理画面の `一般公開へ昇格` で公開用R2へコピーする
4. 昇格公開は標準7日で自動的に一般一覧から消える
5. 既に一般公開中の作品は `延長（+7日）` で現在の期限から7日延長する
6. 管理画面では `一般公開中（あとN日）` として残り日数を確認する
7. 昇格時と延長時は `PUBLIC` 入力確認を通し、誤操作を避ける

R2の推奨分離:

```text
tsukuyomi-reader-books
  限定レビュー用。未発表稿、賞応募候補、認証ログ、限定レビュー読者リストを置く。

tsukuyomi-reader-public-books
  一般公開用。昇格コピーされた本文、表紙、公開manifestだけを置く。
```

一般公開用Pagesは `tsukuyomi-reader-public-books` だけを読みます。限定レビュー用Pagesは `tsukuyomi-reader-books` を読み、管理画面の昇格操作だけが公開用R2へコピーします。
