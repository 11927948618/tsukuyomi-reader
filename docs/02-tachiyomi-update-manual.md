...
- 発行パスワードは7日更新を目安に管理します。実際には再発行または無効化するまで使えます
- ログインセッションは14日で期限切れ
- 同じメールアドレスまたは仮IDで10分以内に複数セッションが動いた場合、管理画面の認証イベントに `同時利用検知` を残す
- 有効IDのPW失敗、ロック、PW発行/無効化、同時利用検知だけを詳細イベントに残し、未知IDへの試行は件数集計だけにする。旧仕様のPW期限切れ記録が残る場合があります
...
この設定はメールアドレスを含む個人情報ログになります。読者同士に進捗を公開するものではありませんが、限定レビューの案内文やCloudflare Accessの案内文に、管理側が閲覧データを一元保管・集計し、文芸分析目的で使うことがある旨を明記してください。
...
- 標準期限は7日
- 既に一般公開中の作品は、ボタンが `延長（+7日）` になり、現在の期限から7日延長する
- 期限切れ後に延長した場合は、今日から7日間の一般公開に戻す
- 管理画面では `一般公開中（あとN日）` として残り日数を表示する
...
表紙だけ削除したい場合は、作品一覧の「表紙削除」を押します。作品本文と作品情報は残り、表紙だけ未設定になります。

作品そのものを消す場合は、作品一覧の「作品削除」を押します。作品一覧から外したあと、現在その作品が参照している本文ファイルと表紙ファイルをR2から削除します。過去に差し替えで残った古いファイルは、R2使用状況の内訳を確認しながらCloudflare側で整理します。
