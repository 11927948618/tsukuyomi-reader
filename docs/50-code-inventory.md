...
### C-2. 大容量書籍の描画（最大30万字）  ← 設計中 / 一部着手
- **判断（2026-09-07）**: ユーザー想定は最大30万字。必要。設計メモ `docs/52-large-book-rendering.md`。
- **着手済み**:
  - v0.1.228 ページャの**トークン化キャッシュ**（設定変更時の再ページ化 約420ms→約35ms）。
  - v0.1.229 **initReader ティアダウン**（window リスナー蓄積の解消）＋ **EPUB/PDF blob URL 解放**。
- **残り（別セッション）**: `docs/52` ステップ3〜8 —
  トークン化の async 化・プログレッシブ初期表示・**スクロールモードのページ・ウィンドウ化**（案A、本丸）・
  document model の遅延構築。§4 に要判断（既定の並べ方は決定済み、進捗UI・目標時間が残る）。
- 計測: ページ切替は DOM 軽量（現在ページのみ）。**スクロールモードが本丸**（約23万字で全文注入 → 秒単位）。
