...
- Reader内パスワード認証では、管理画面の `限定レビュー認証管理` から読者を登録し、閲覧許可とパスワード発行をまとめて実行できます
- 読者はメールアドレスまたは仮IDとパスワードでログインできます
- Reader内パスワード認証は、ログイン総当たり対策、管理上の7日更新目安、同時利用検知、限定レビュー時の本文キャッシュ抑制を行います。発行済みパスワードは再発行または無効化するまで使えます
...
- ページ切替 / 横スクロール / 縦スクロール が切り替わること
- TXTの構造自動判別をオン / オフできること
- 設定保存後に再度開き、表示設定が反映されること
- 栞がある本を再度開き、栞から読むか最初から読むかを選べること
